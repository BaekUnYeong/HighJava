...
public class T01_VectorTest {
	public static void main(String[] args) {
		...
		// remove(삭제할 데이터) => '삭제할데이터'를 찾아서 삭제한다.
		...
		System.out.println("삭제후(new Integer('a') : " + v1);
		System.out.println();
		 ...
		 * 제너릭 타입(generic type)
		 * 
		 * - Collection 객체를 선언할 때 <>안에 그 COllections이 저장할 데이터 타입을 정해주는 것을 말한다.
		 * - 이런식으로 선언하게 되면 그 데이터 타입은 클래스이어야한다.
		...
		Vector<Integer> v3 = new Vector<>();	//Integer만 저장가능 <> 안에 Integer 생략 가능
		
		v2.add("안녕하세요.");
		// v2.add(123) : //오류 : 다른 종류의 데이터를 추가할 수 없다.
		
		String temp2 = v2.get(0);
//		System.out.println("v2.add 후 데이터 => " + v2.size());
		...
		v2.clear();
//		System.out.println("v2.clear 후 데이터 => " + v2);
		...
		// 벡터의 데이터들을 순서대로 모두 가져와서 사용하고 싶으면 반복움을 사용하면 된다(주로 for문을 사용함)
		 ...
		 *  처리할 내용들;
		 *  }
		 *  => 주어진 '배열변수나 Collection계열 변수'의 데이터 개수 만듬 반복함.
		 *  => 반복할 때마다 '배열변수나 Collection계열변수'의 데이터를 하나씩 꺼내와 '변수명'에 저장한 후 '처리할 내용들을 처리한다.
		 */
