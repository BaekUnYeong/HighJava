...
public class T02_EnumTest {
	...
	public enum Season {
		...
		private String str; // 괄호속의 값이 저장될 변수 선언

		Season(String data) // 생성자. 열거형 생성자의 제어자는 private이다.
		{
			str = data;
		}
	...
	public static void main(String[] args) {
		/*
		 * 열거형에서 사용되는 메서드 1. name() => 열거형 상수의 이름을 문자열로 반환한다. 2. ordinal() => 열거형 상수가
		 * 정의된 순서값을 반환한다.(0부터 시작함) 3. valueOf("열거형상수이름"); => 지정된 열거형에서 '열거형상수이름'과 일치하는
		 * 열거형 상수를 반환한다.
		 */
		City myCity1;
		City myCity2;
		...
		// 열거형 객체변수에 값 저장하기
		myCity2 = City.서울;
		myCity1 = City.valueOf("서울"); // City enum에서 '서울'상수를 가져온다.
